...
public class Utility {
    ...
    /**
     * Set the country code into shared preference. This function should not be called from
    ...
    public static void setPreferredLocation(Context context, String location) {
        ...
        spe.putString(context.getString(R.string.pref_location_key), location);
        spe.commit();
    }

    public static String getPreferredCountryCode(Context context) {
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);
        return sp.getString(context.getString(R.string.pref_country_code_key),
     ...
     */
    public static void setPreferredCountryCode(Context context, String countryCode) {
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);
        SharedPreferences.Editor spe = sp.edit();
        spe.putString(context.getString(R.string.pref_country_code_key), countryCode);
        spe.commit();
    }
     ...
     */
    static public void setLocationStatus(Context c, @SunshineSyncAdapter.LocationStatus int locationStatus) {
    ...
    public static String getFriendlyDayString(Context context, long dateInMillis, boolean displayLongToday) {
        ...
        if (displayLongToday && julianDay == currentJulianDay) {
            ...
            SimpleDateFormat shortenedDateFormat = new SimpleDateFormat(context.getString(R.string.format_friendly_day));
    ...
    public static String getDayName(Context context, long dateInMillis) {
        ...
        if (julianDay == currentJulianDay) {
            ...
            String capitalizeDay = dayFormat.format(dateInMillis);
            if (capitalizeDay.length() == 0)
                return capitalizeDay;
            else
                return capitalizeDay.substring(0, 1).toUpperCase() + capitalizeDay.substring(1);
        }
    }
    ...
    public static String getFormattedMonthDay(Context context, long dateInMillis ) {
        ...
        SimpleDateFormat monthDayFormat = new SimpleDateFormat(context.getString(R.string.format_month_day));
     ...
     * @return resource id for the corresponding icon. -1 if no relation is found.
     */
    public static int getArtResourceForWeatherConditionNotification(int weatherId) {
        // Based on weather code data found at:
        // http://bugs.openweathermap.org/projects/api/wiki/Weather_Condition_Codes
        if (weatherId >= 200 && weatherId <= 232) {
            return R.drawable.ic_stat_art_storm;
        } else if (weatherId >= 300 && weatherId <= 321) {
            return R.drawable.ic_stat_art_light_rain;
        } else if (weatherId >= 500 && weatherId <= 504) {
            return R.drawable.ic_stat_art_rain;
        } else if (weatherId == 511) {
            return R.drawable.ic_stat_art_snow;
        } else if (weatherId >= 520 && weatherId <= 531) {
            return R.drawable.ic_stat_art_rain;
        } else if (weatherId >= 600 && weatherId <= 622) {
            return R.drawable.ic_stat_art_snow;
        } else if (weatherId >= 701 && weatherId <= 761) {
            return R.drawable.ic_stat_art_fog;
        } else if (weatherId == 761 || weatherId == 781) {
            return R.drawable.ic_stat_art_storm;
        } else if (weatherId == 800) {
            return R.drawable.ic_stat_art_clear;
        } else if (weatherId == 801) {
            return R.drawable.ic_stat_art_light_clouds;
        } else if (weatherId >= 802 && weatherId <= 804) {
            return R.drawable.ic_stat_art_clouds;
        }
        return -1;
    }
